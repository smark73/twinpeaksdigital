...
?>
	<p><label><?php $form->add_radio( 'method', 'add' ); ?> <?php _e( '<strong>Add Layouts:</strong> Add the Layouts from this export to this site\'s existing Layouts. The Views in this export will not be imported to this site.', 'it-l10n-Builder-Cohen' ); ?></label></p>
	<p><label><?php $form->add_radio( 'method', 'custom' ); ?> <?php printf( __( '<strong>Custom:</strong> Selectively choose how to import the %s.', 'it-l10n-Builder-Cohen' ), $name ); ?></label></p>
	<p><label><?php $form->add_radio( 'method', 'replace' ); ?> <?php printf( __( '<strong>Replace:</strong> Delete the %1$s from this site and replace with the %1$s in the export file.', 'it-l10n-Builder-Cohen' ), $name ); ?></label></p>
	<p><label><?php $form->add_radio( 'method', 'skip' ); ?> <?php printf( __( '<strong>Skip:</strong> Do not import %1$s from the export file. The site\'s %1$s will remain unchanged.', 'it-l10n-Builder-Cohen' ), $name ); ?></label></p>
<?php
		...
		function show_import_customizations_form( $form, $info, $data, $post_data ) {
			...
			echo '<h4>' . __( 'Select Layouts from the Import File to Add to Site', 'it-l10n-Builder-Cohen' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Checking a layout below will add that layout from the import file to the site.', 'it-l10n-Builder-Cohen' ) . "</p>\n";
...
<?php
			...
			echo '<h4>' . __( 'Select Layouts from the Site to Keep', 'it-l10n-Builder-Cohen' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Unchecking one of the following layouts will remove it from the site.', 'it-l10n-Builder-Cohen' ) . "</p>\n";
...
<?php
			...
			echo '<h4>' . __( 'Select Views from the Import File to Add to Site', 'it-l10n-Builder-Cohen' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Checking a view below will add that view from the import file to the site.', 'it-l10n-Builder-Cohen' ) . "</p>\n";
...
?>
	...
	<?php elseif ( false === $description['failed'] ) : ?>
		<?php $description = sprintf( __( '<strong>%1$s</strong> uses the export file\'s &quot;%2$s&quot; layout', 'it-l10n-Builder-Cohen' ), $description['description'], $description['layout'] ); ?>
		<p><label><?php $form->add_check_box( $id ); ?> <?php echo $description; ?></label></p>
	<?php else : ?>
		<p><?php $form->add_check_box( $id, array( 'disabled' => 'disabled' ) ); ?> <?php printf( __( '<span style="color:red;">%s</span> &mdash; This view does not exist on the site.', 'it-l10n-Builder-Cohen' ), $description['description'] ); ?></p>
	<?php endif; ?>
<?php
			...
			echo '<h4>' . __( 'Select Views from the Site to Keep', 'it-l10n-Builder-Cohen' ) . "</h4>\n";
			echo "<div style='margin-left:20px;'>\n";
			echo '<p>' . __( 'Unchecking one of the following views will remove it from the site.', 'it-l10n-Builder-Cohen' ) . "</p>\n";
...
?>
	<?php if ( ! is_wp_error( $description ) ) : ?>
		<?php $description = sprintf( __( '<strong>%1$s</strong> uses the site\'s &quot;%2$s&quot; layout', 'it-l10n-Builder-Cohen' ), $description['description'], $description['layout'] ); ?>
		...
		function _get_view_description( $id, $data, $meta = null ) {
			...
			if ( false !== strpos( $id, '|' ) ) {
				...
				if ( 'builder_is_custom_post_type' == $function )
					$description = sprintf( __( 'Custom post type: %s', 'it-l10n-Builder-Cohen' ), $arg );
				else if ( 'builder_is_custom_taxonomy' == $function )
					$description = sprintf( __( 'Custom taxonomy: %s', 'it-l10n-Builder-Cohen' ), $arg );
...
/*			if ( 'replace' === $post_data['method'] )
			...
			$setting_descriptions = array(
				'description'   => __( 'Layout Name', 'it-l10n-Builder-Cohen' ),
				'width'         => __( 'Layout Width', 'it-l10n-Builder-Cohen' ),
				'custom_width'  => __( 'Layout Width', 'it-l10n-Builder-Cohen' ),
				'extension'     => __( 'Extension', 'it-l10n-Builder-Cohen' ),
				'disable_style' => __( 'Extension Style', 'it-l10n-Builder-Cohen' ),
				'modules'       => __( 'Modules', 'it-l10n-Builder-Cohen' ),
			);
...
?>
	<div style="margin:20px;">
		<p><label><?php $form->add_radio( 'actions[]', 'new_guid' ); ?> <?php _e( '<strong>New ID:</strong> Create a new ID for the Layout being imported.', 'it-l10n-Builder-Cohen' ); ?></label></p>
		<p><label><?php $form->add_radio( 'actions[]', 'replace' ); ?> <?php printf( __( '<strong>Replace:</strong> Remove this site\'s &quot;%s&quot; Layout and replace it with this one for the import.', 'it-l10n-Builder-Cohen' ), $layout_settings['layouts'][$guid]['description'] ); ?></label></p>
		<p><label><?php $form->add_radio( 'actions[]', 'skip' ); ?> <?php _e( '<strong>Skip:</strong> Do not import this Layout.', 'it-l10n-Builder-Cohen' ); ?></label></p>
	</div>
<?php
...
?>
	<div style="margin:20px;">
		<p><label><?php __( 'New Name:', 'it-l10n-Builder-Cohen' ); ?> <?php $form->add_text_box( 'new_description' ); ?></label></p>
...
?>
	<div style="margin:20px;">
		<p><label><?php _e( 'Select a Layout for this View:', 'it-l10n-Builder-Cohen' ); ?> <?php $form->add_drop_down( 'set_layout', $layout_options ); ?></label></p>
	</div>
<?php
...
?>
<div style="margin:20px;">
	<p><label><?php _e( 'Select a Default Layout:', 'it-l10n-Builder-Cohen' ); ?> <?php $form->add_drop_down( 'default_layout', $layout_options ); ?></label></p>
</div>
<?php
...
?>
	<div title="title - tooltip test" class="builder_tooltip"><strong><?php _e( 'Name:', 'it-l10n-Builder-Cohen' ); ?></strong> <?php echo $data['description']; ?></div>
	<div><strong><?php _e( 'Width:', 'it-l10n-Builder-Cohen' ); ?></strong> <?php echo $data['width']; ?> pixels</div>
	<div><strong><?php _e( 'Modules:', 'it-l10n-Builder-Cohen' ); ?></strong></div>
	<ul>
