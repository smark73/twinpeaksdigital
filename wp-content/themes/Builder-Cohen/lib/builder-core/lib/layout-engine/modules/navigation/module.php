...
if ( ! class_exists( 'LayoutModuleNavigation' ) ) {
	class LayoutModuleNavigation extends LayoutModule {
		...
		function LayoutModuleNavigation() {
			...
			$this->_name = _x( 'Navigation', 'module', 'it-l10n-Builder-Cohen' );
			$this->_description = __( 'This module adds a horizontal navigation bar. Category and Page navigations are available.', 'it-l10n-Builder-Cohen' );
		...
		function show_conflicts_form( $form, $data ) {
			...
			echo '<p>' . sprintf( __( 'A new Navigation Type must be selected for the Navigation module (original setting: %s).', 'it-l10n-Builder-Cohen' ), $original_type ) . "</p>\n";
...
?>
	<p><?php printf( __( 'The Layout Module offers a number of different options for adding navigation elements to your layout. To get the most power out of this feature, use WordPress\' built-in <a href="%s" target="_blank">Menu Editor</a> to create your menus. You can then select to use one of these menus in the Navigation Module.', 'it-l10n-Builder-Cohen' ), admin_url( 'nav-menus.php' ) ); ?></p>
	<p><?php printf( __( 'The Legacy Menu Types are present as a fallback in case you have not configured any menus in the <a href="%1$s" target="_blank">Menu Editor</a>. The Builder menu types can be configured in the <a href="%2$s" target="_blank">Builder\'s Theme Settings</a>. The WordPress Pages option uses WordPress\' built-in <code>wp_list_pages</code> function to show a listing of all of your site\'s pages.', 'it-l10n-Builder-Cohen' ), admin_url( 'nav-menus.php' ), admin_url( 'admin.php?page=theme-settings#theme-settings-basic-menu_builder' ) ); ?></p>
<?php
...
?>
	<tr><td><label for="type"><?php _e( 'Navigation Type', 'it-l10n-Builder-Cohen' ); ?></label></td>
...
<?php
		...
		function _get_menu_types() {
			...
			$types[__( 'Legacy Menu Types', 'it-l10n-Builder-Cohen' )] = array(
				'categories' => __( 'Builder Settings Categories', 'it-l10n-Builder-Cohen' ),
				'pages'      => __( 'Builder Settings Pages', 'it-l10n-Builder-Cohen' ),
				'wp_legacy'  => __( 'WordPress Pages', 'it-l10n-Builder-Cohen' ),
			);
